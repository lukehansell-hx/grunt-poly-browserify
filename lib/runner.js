...
Runner.prototype.runBundler = function (brand, options, next){
  ...
  this.__applyTransforms(b, options.transforms);
  this.__applyRequires(b, options.requires);
  this.__applyExternals(b, options.externals);
  this.__applyIgnores(b, options.ignores);
  this.__applyExcludes(b, options.excludes);
  ...
  b.bundle(function(err, buf){
      if(err || !buf){
        ...
        self.grunt.verbose.writeln('Writing to file ' + options.dest);
